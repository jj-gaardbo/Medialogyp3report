...
\todo{visualisation of bounding box vs center of mass as, also explanation} 
also explanation of how rotation and stuff, maybe put that in the design section.
Finally, since we find way more blobs than there are actual circles the boolean \codeword{returnable} exists and returning all the blobs to . It is only set to true if the object is 
\subsection{Implementation}
First one loops through each pixel in the image and checks if it is white. Obviously this only works if the image is binary. In our case, the image is 8bit greyscale, mostly to help visualize the blobs for us humans. If the pixel is white, create a blob, add it the list of blobs, and start the fire at the pixel. This is implemented as shown in Listing \autoref{listing:grassfire}.
\begin{listing}[H]
