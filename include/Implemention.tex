...
>>>>>>> 7467bfea06091b28a28a62db51a59591e8dfb6a9
\begin{center}
	\begin{multicols}{2}
...
for (int i = 0; i < nRows; i += GRIDSIZE) {
	...
	\end{minted}
\end{listing}
As can be seen in \ref{listing:sum}, the code starts by saving the
values of each channel to the corresponding variables. Then these values are summed. Now we have the two variables that we need to use the lookup table. Since the sum is not going to change, we get the address of the row that our sum is on and save it in \codeword{lutptr}. Now we can assign the values using \codeword{lutptr}. In accordance with the therory section blue is useful data and so does not need to be assigned\todo{remove blue man, also do so in code}, and note that would not need to assign blue, we do it when we want to display the output. But in the final code it can be omitted
\section{Limitations}
