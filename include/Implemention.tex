...
The integer \codeword{nr} is used to help visualize the grass-fire algorithm, and will later contain the found value from the pattern underneath the markers. Finally, since we find way more blobs than there are actual markers, and returning all the blobs to unity be a waste of time boolean \codeword{returnable} exists. It is only set to true if the blob passes all test and therefore is considered a marker. This object is used to represent each blob and is used in the grassfire algorithm.
\subsection{Grassfire implementation}
Our implementation can be seen in listing \ref{listing:grassfireIm}. Due to its iterative nature, this method will be called many thousand times. So it is important to cut down on variable instantiation and general computation as much as possible. We have made attempts to do this in the function. Starting with the method signature where we pass by reference whenever possible. While there is very little difference between passing integers and references, you do not have to instantiate the reference every call, so it is slightly faster. Another thing to note is that the function does not make us of the very computationally expensive Mat.at<uchar>(y,x) function. Instead we use pointers 
\begin{listing}[H]
	\caption{The drop fire function starts from one pixel and recursively spreads until the entire blob has been consumed. It does this through connected component analysis, analyzing its connected pixels, and spreading to them if they are white.}
	\begin{minted}[frame=lines, framesep=2mm,baselinestretch=1.1,fontsize=\footnotesize,linenos]{c++}
...
\subsection{Scanning for white pixel}
First one loops through each pixel in the image and checks if it is white. Obviously this only works if the image is binary. In our case, the image is 8bit greyscale to help visualize the blobs for us humans. If the pixel is white, create a blob, add it the list of blobs, and start the fire at the pixel. This is implemented as shown in Listing \autoref{listing:grassfire}.
\begin{listing}[H]
