\chapter{Analysis}
	...
	\subsection{Expert Interviews}
		...
		As previously stated, the interviews were conducted over the phone. A project group representative e-mailed 12 landscape architects throughout the country and managed to make appointments with 4 experts. During the e-mail correspondence the experts were informed that the interviews would be recorded and used in the report.
		
		The expert interviews were later transcribed\todo{Insert transcriptions in appendencies and refer to them here} and analysed using traditional coding to collect qualitative data that would later be categorized in order to :
		\todo{Write more here when analysis of interviews is done}
    ...
    \section{State of the art}\label{sec:SOTA}
		...
		\subsection{SketchUp}
			...
			\subsection{Oculus Go}
				Oculus Go\footnote{Oculus Go: \url{https://www.oculus.com/go/}} is a virtual reality headset set to launch in the first quarter of 2018. Made by Oculus known for the Oculus Rift, the Go implements similar features to the Rift with virtual reality games, social applications and different kinds of 360 degree experiences overall. Furthermore, the Go is designed to be lightweight with comfortable fabrics for usage during long sessions. Unlike the Rift, Oculus Go is wireless and can therefore be taken anywhere, which enables it to be used in a broader context eg. for outdoor og travelling use. It uses a simple controller for navigation and action to make for more complex interaction	within the featured applications, although it does not feature fully tracked motion controlling. Another strength the Go brings, is the built in spatial audio, which alongside the 2560x1440 pixel display allows the Go to be a contender to other immersive VR headsets on the market. //
				\begin{figure}[H]
					\centering
					\includegraphics[width=0.6\linewidth]{figure/Analysis/oculusgo}
